...
# 該地區這週寫進 sheet 的文章數若低於門檻，從被 require_funding 濾掉的候補池（真實抓到、
# 只是標題沒有硬融資字眼的文章，例如人物專訪、公司深度報導）補滿，而不是放寬濾網本身。
# 候補池一樣會走正常的 Stage0/Stage1+2 AI 判斷，AI 判斷不是新創的一樣會被拒絕，只是多花
# 幾次 LLM 呼叫，不影響最終判斷品質。
REGION_AI_TARGET = {"台灣": 30}
